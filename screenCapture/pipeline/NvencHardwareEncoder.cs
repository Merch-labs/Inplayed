...
public sealed class NvencHardwareEncoder : IHardwareEncoder
{
	...
	private const int MaxRegisteredResources = 32;
	private const uint NvEncPicFlagForceIntra = 0x1;
	private const uint NvEncPicFlagForceIdr = 0x2;
	private const uint NvEncPicFlagOutputSpsPps = 0x4;
	private readonly object _sync = new();
	private readonly Dictionary<nint, RegisteredResourceEntry> _registeredResources = new();
	...
	private long _mappedFrameCount;
	private int _bitstreamWriteIndex;
	private long _submittedEncodeCount;
	private int _encodeSubmitFailures;
	private int _lastEncodePictureRc;
	private int _encodeFrameIndex;
	private long _lastInputTimestamp;
	private int _inputTimestampBacktracks;
	private bool _forceNextIdr;
	private long _forcedIdrCount;
	private int _lockBitstreamFailures;
	private int _lockBitstreamBusyCount;
	private int _lockBitstreamRetryableCount;
	private int _lockBitstreamHardFailureCount;
	...
	private int _pendingDrainTimeouts;
	private long _drainPassCount;
	private long _flushRequestCount;
	private long _flushEmptySnapshotCount;
	private long _flushRecoveredCount;
	private int _registeredResourceEvictions;
	private int _resourceUseTick;
	...
	public void Encode(TextureFrameRef frame)
	{
		lock (_sync)
		{
			...
			var encodePicture = _encodePicture;
			if (mapInput == null || unmapInput == null || encodePicture == null)
			{
				return;
			}
			...
			try
			{
				...
				if (_bitstreamBuffers.Length == 0)
				{
					_encodeSubmitFailures++;
				}
				else
				{
					var bitstreamBuffer = _bitstreamBuffers[_bitstreamWriteIndex];
					var normalizedTimestamp = NormalizeInputTimestamp(frame.Timestamp);
					var encodePicFlags = 0u;
					if (_forceNextIdr)
					{
						encodePicFlags = NvEncPicFlagForceIntra | NvEncPicFlagForceIdr | NvEncPicFlagOutputSpsPps;
					}
					var picParams = new NvencNative.NV_ENC_PIC_PARAMS
					{
						version = NvencNative.EncodeStructVersion<NvencNative.NV_ENC_PIC_PARAMS>(NvencNative.NV_ENC_PIC_PARAMS_VER),
						inputWidth = (uint)frame.Width,
						inputHeight = (uint)frame.Height,
						inputPitch = (uint)(frame.Width * 4),
						encodePicFlags = encodePicFlags,
						frameIdx = (uint)_encodeFrameIndex,
						inputTimeStamp = new IntPtr(normalizedTimestamp),
					...
					else
					{
						...
						_bitstreamTimestamps[_bitstreamWriteIndex] = normalizedTimestamp;
		...
		if (snapshot.Packets.Count == 0)
		{
			if (wasRunning)
			{
				lock (_sync)
				{
					if (_running)
					{
						DrainPendingBitstreams(maxPasses: 24, doNotWait: false);
					}
				}
	...
	public void Stop()
	{
		lock (_sync)
		{
			...
			_running = false;
			var pending = CountPendingBitstreams();
			var stopDrainPasses = Math.Clamp(Math.Max(16, pending * 4), 16, 256);
			DrainPendingBitstreams(maxPasses: stopDrainPasses, doNotWait: false);
	...
	public void Dispose()
	{
		lock (_sync)
		{
			...
			_registeredResourceEvictions = 0;
			_resourceUseTick = 0;
			...
			_mappedFrameCount = 0;
			_bitstreamWriteIndex = 0;
			_submittedEncodeCount = 0;
			_encodeSubmitFailures = 0;
			_lastEncodePictureRc = 0;
			_encodeFrameIndex = 0;
			_lastInputTimestamp = 0;
			_inputTimestampBacktracks = 0;
			_forceNextIdr = false;
			_forcedIdrCount = 0;
			_lockBitstreamFailures = 0;
			_lockBitstreamBusyCount = 0;
			_lockBitstreamRetryableCount = 0;
			_lockBitstreamHardFailureCount = 0;
			...
			_pendingDrainTimeouts = 0;
			_drainPassCount = 0;
			_flushRequestCount = 0;
			_flushEmptySnapshotCount = 0;
			_flushRecoveredCount = 0;
			_running = false;
	...
	private long NormalizeInputTimestamp(long inputTimestamp)
	{
		if (inputTimestamp <= 0)
		{
			inputTimestamp = _lastInputTimestamp + 1;
		}

		if (inputTimestamp <= _lastInputTimestamp)
		{
			_inputTimestampBacktracks++;
			inputTimestamp = _lastInputTimestamp + 1;
		}
	...
	private bool TryDrainBitstream(IntPtr bitstreamBuffer, long frameTimestamp, bool doNotWait)
	{
		...
		try
		{
			...
			if (lockRc != 0)
			{
				if (IsRetryableLockFailure(lockRc, doNotWait))
				{
					_lockBitstreamBusyCount++;
					_lockBitstreamRetryableCount++;
				}
				else
				{
					_lockBitstreamFailures++;
					_lockBitstreamHardFailureCount++;
				}
	...
	private bool EnsureRegisteredResource(TextureFrameRef frame, out IntPtr registeredResource)
	{
		...
		{
			existing = existing with { LastUseTick = ++_resourceUseTick };
			_registeredResources[texturePtr] = existing;
			registeredResource = existing.RegisteredResource;
			return true;
		}
		...
		try
		{
			...
			_registeredResources[texturePtr] = new RegisteredResourceEntry(registeredResource, ++_resourceUseTick);
			EvictRegisteredResourcesIfNeeded();
	...
	private void UnregisterAllResources()
	{
		...
		{
			var rc = unregister(_encoderSession, kv.Value.RegisteredResource);
			if (rc != 0)
			{
	...
	private void EvictRegisteredResourcesIfNeeded()
	{
		if (_registeredResources.Count <= MaxRegisteredResources || _unregisterResource == null || _encoderSession == IntPtr.Zero)
		{
			return;
		}

		var overshoot = _registeredResources.Count - MaxRegisteredResources;
		if (overshoot <= 0)
		{
			return;
		}

		var keysToEvict = _registeredResources
			.OrderBy(kv => kv.Value.LastUseTick)
			.Take(overshoot)
			.Select(kv => kv.Key)
			.ToArray();

		for (var i = 0; i < keysToEvict.Length; i++)
		{
			var key = keysToEvict[i];
			if (!_registeredResources.TryGetValue(key, out var entry))
			{
				continue;
			}

			var rc = _unregisterResource(_encoderSession, entry.RegisteredResource);
			if (rc != 0)
			{
				_unregisterResourceFailures++;
				continue;
			...
			_registeredResources.Remove(key);
			_registeredResourceEvictions++;
		}
	}

	private readonly record struct RegisteredResourceEntry(IntPtr RegisteredResource, int LastUseTick);
